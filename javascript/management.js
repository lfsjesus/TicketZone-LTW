...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tbody.addEventListener('change', (event) => {
        ...
        if (target.tagName === 'SELECT') {
            ...
            const user_id = target.closest('tr').querySelector('input[type="hidden"]').value;
        ...
        if (this.status == 200) {
            ...
            if (response.users.length > 0) {
                response.users.forEach(person => {
                        ...
                        <select name="department_id">
                            ...
                            ${
                                response.departments.map(department => {
                                    return `<option value="${department.id}" ${person.department && department.id == person.department.id ? 'selected' : ''}>${department.name}</option>`
                                }
                                ).join('')
                            }
                    </td>
                    <td>
                        <select name="type">
                        <option value="client" ${person.type == 'client' ? 'selected' : ''}>Client</option>
                        <option value="agent" ${person.type == 'agent' ? 'selected' : ''}>Agent</option>
                        <option value="admin" ${person.type == 'admin' ? 'selected' : ''}>Admin</option>
                        </select>
                    </td>
                    <td>
                        <form action="../actions/action_delete_user.php" method="post">
                            <button class="delete" name="id" value="${person.id}" type="submit"><span class="material-symbols-outlined">delete</span></button>
                        </form>
                    </td>
                    `;
